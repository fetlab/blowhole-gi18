	...
	while len(node.possible_size):
		...
		for move_dy in tube_length:
			if not TestFit(node.object,cur_frame,[size,size,size],move_dy):
				...
				drill_holes(copy_object,cur_frame,pipe_filename,[1,0.1*move_dy+0.1,1],0,0)
				drill_holes(copy_object,cur_frame,hole_filename,[size,size,size],move_dy,1)
